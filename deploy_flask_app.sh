...
User=root
Group=www-data
WorkingDirectory=/root/flask_deploy_script
ExecStart=/root/flask_deploy_script/venv/bin/gunicorn --workers 3 --bind unix:/root/flask_deploy_script/myflaskapp.sock -m 000 app:app
...
sudo chmod 777 /root/flask_deploy_script/myflaskackapp.sock
sudo chmod a+x /root
...
server {
    ...
    location / {
        ...
        proxy_pass http://unix:/root/flask_deploy_script/myflaskapp.sock;
    }
}
EOF
